...
class CompactString:
    ...
    def __le__(self, other): # <=
        ...
        right = other.data.header.next
        equals = True
        while (left and left.data and right and right.data) is not None:
            if left.data != right.data:
                equals = False
            left = left.next
            right = right.next
        return (equals and (left.data == right.data)) or (self < other) 
    ...
    def __repr__(self):
        ...
        a = self.data.header.next
        ret = ''
        while a.data:
            ret += (a.data[0] * a.data[1])
            a = a.next
        return ret
